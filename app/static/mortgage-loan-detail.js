...
function renderArmDetailSection(loan) {
  ...
  document.getElementById("modal-arm-product").textContent = armProductLabel(
    loan,
  );
  document.getElementById("modal-arm-margin").textContent = fmtPct(arm.margin);
  document.getElementById("modal-arm-caps").textContent =
    (arm.cap_initial || 0) + "/" + (arm.cap_periodic || 0) + "/" +
    (arm.cap_lifetime || 0);
  document.getElementById("modal-arm-fixed-period").textContent =
    (arm.fixed_period_years || 0) + " yr";
  ...
  document.getElementById("modal-arm-first-adj").textContent = firstAdjDate
  ...
  var scenarios = [
    {
      key: "worst",
      rateId: "modal-arm-rate-worst",
      piId: "modal-arm-pi-worst",
    },
    {
      key: "intro_forever",
      rateId: "modal-arm-rate-intro",
      piId: "modal-arm-pi-intro",
    },
  ...
  scenarios.forEach(function (sc) {
    ...
    var finalPi = finalSeries.length
...
function openLoanModal(id) {
  ...
  document.getElementById("modal-loan-amount").textContent = fmtCurrency(
    loan.loan_amount,
  );
  document.getElementById("modal-interest").textContent = fmtPct(loan.interest);
  document.getElementById("modal-term").textContent = (loan.term_years || "—") +
    " yr";
  document.getElementById("modal-pi").textContent = fmtCurrency(loan.pi);
  document.getElementById("modal-monthly-total").textContent = fmtCurrency(
    loan.monthly_total,
  );
  document.getElementById("modal-points").textContent = loan.points
    ? loan.points.toFixed(3) + " pts"
    : "—";
  document.getElementById("modal-net-credits").textContent = fmtCurrency(
    loan.net_credits,
  );
  document.getElementById("modal-net-fees").textContent = fmtCurrency(
    loan.net_fees,
  );
  document.getElementById("modal-closing").textContent = fmtCurrency(
    loan.closing,
  );
  document.getElementById("modal-prop-tax").textContent = fmtCurrency(
    loan.prop_tax,
  );
  document.getElementById("modal-hoa").textContent = fmtCurrency(loan.hoa);
  document.getElementById("modal-insurance").textContent = fmtCurrency(
    loan.insurance,
  );
...
document.getElementById("loan-detail-modal").addEventListener(
  "close",
  function () {
    var modal = document.getElementById("loan-detail-modal");
    var panel = document.getElementById("preview-panel");
...
(function () {
  ...
  document.addEventListener("mousemove", function (e) {
    ...
    var x = Math.max(
      0,
      Math.min(e.clientX - ox, window.innerWidth - panel.offsetWidth),
    );
    var y = Math.max(
      0,
      Math.min(e.clientY - oy, window.innerHeight - panel.offsetHeight),
    );
