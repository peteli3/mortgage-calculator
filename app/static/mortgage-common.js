...
function normalizeLoan(loan) {
    if (!loan) return loan;
    var out = {};
    for (var k in loan) if (Object.prototype.hasOwnProperty.call(loan, k)) out[k] = loan[k];
...
function showToast(message, type) {
    ...
    wrap.innerHTML = '<div class="alert ' + alertClass + ' text-sm py-2 px-4">' + escHtml(message) + '</div>';
...
function renderArmPreview(loan) {
    ...
    Object.keys(ids).forEach(function(sc) {
        ...
        if (el) {
            ...
            if (fp.title) { el.title = fp.title; } else { el.removeAttribute('title'); }
        }
    });
}
...
function calculateAmortizationV2(loan, scenario, startDate, extraPayments) {
    ...
    for (var month = 1; month <= totalPayments && balance > 0.01; month++) {
        ...
        if (rateThis !== lastRate) {
            currentPayment = computePayment(balance, rateThis, totalPayments - (month - 1));
            lastRate = rateThis;
        }
...
function computeLoanScenarioStats(loan) {
    var introPi = computePayment(loan.loan_amount || 0, loan.interest || 0, Math.round((loan.term_years || 30) * 12));
    if (loan.loan_type !== 'arm' || !loan.arm) {
